...
function bug_group_action_init($p_action)
{
	...
	if (
		!isset($t_valid_actions[$t_action]) &&
...
function bug_group_action_get_commands(array $p_project_ids = null)
{
	...
	foreach ($p_project_ids as $t_project_id) {
		...
		if (
			!isset($t_commands['CLOSE']) && $t_update_bug_status_allowed &&
		...
		if (
			!isset($t_commands['RESOLVE']) && $t_update_bug_status_allowed &&
		...
		if (
			!isset($t_commands['EXT_UPDATE_PRODUCT_BUILD']) &&
		...
		if (
			!isset($t_commands['EXT_ATTACH_TAGS']) &&
		...
		if (
			!isset($t_commands['UP_PRODUCT_VERSION']) &&
		...
		if (
			!isset($t_commands['UP_FIXED_IN_VERSION']) &&
			version_should_show_product_version($t_project_id) && $t_update_bug_allowed
		) {
		...
		if (
			!isset($t_commands['UP_TARGET_VERSION']) &&
