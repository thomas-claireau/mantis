...
function filter_ensure_valid_filter(array $p_filter_arr)
{
	...
	if (
		$p_filter_arr['_view_type'] == FILTER_VIEW_TYPE_ADVANCED
	...
	if (
		$p_filter_arr['_view_type'] == FILTER_VIEW_TYPE_SIMPLE
		&& !filter_field_is_none($p_filter_arr[FILTER_PROPERTY_HIDE_STATUS])
	) {
	...
	if (!($p_filter_arr[FILTER_PROPERTY_RELATIONSHIP_BUG] > 0
				...
				<div class="widget-header widget-header-small">
					...
					<div id="filter-bar-queries" class="widget-toolbar no-border" style="display: <?php echo $t_collapse_block ? 'block' : 'none' ?>">
						<div class="widget-menu margin-left-8">
							...
							if ($t_is_temporary && $t_can_persist) {
							?>
								<a class="btn btn-primary btn-white btn-round btn-xs" title="<?php echo lang_get('set_as_persistent_filter') ?>" href="<?php echo $t_url_persist_filter ?>">
									<?php print_icon('fa-thumb-tack', 'ace-icon'); ?>
								</a>
							<?php
							}
							?>
							<a class="btn btn-primary btn-white btn-round btn-xs" title="<?php echo lang_get('reset_query') ?>" href="<?php echo $t_url_reset_filter ?>">
						...
						<div class="widget-menu margin-right-8">
							...
							<form method="post" action="view_all_set.php">
								...
								<input id="filter-bar-search-txt" type="text" size="16" class="input-xs" placeholder="<?php echo lang_get('search') ?>" name="<?php echo FILTER_PROPERTY_SEARCH ?>" value="<?php echo string_attribute($t_filter[FILTER_PROPERTY_SEARCH]); ?>" />
								<button id="filter-bar-search-btn" type="submit" name="filter_submit" class="btn btn-primary btn-white btn-round btn-xs" title="<?php echo lang_get('filter_button') ?>">
					...
					<form method="post" name="filters<?php echo $t_form_name_suffix ?>" id="filters_form<?php echo $t_form_name_suffix ?>" action="view_all_set.php">
						...
						<div class="widget-toolbox padding-8 clearfix">
							<div class="btn-toolbar pull-left">
								<div class="form-inline">
									<?php echo '<input type="text" id="filter-search-txt" class="input-sm" size="16" name="', FILTER_PROPERTY_SEARCH, '"', ' placeholder="' . lang_get('search') . '" value="', string_attribute($t_filter[FILTER_PROPERTY_SEARCH]), '" />';
									?>
