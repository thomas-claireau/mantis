...
function project_includes_user($p_project_id, $p_user_id)
{
	$t_query = new DbQuery();
	$t_query->sql(
		'SELECT COUNT(*) FROM {project_user_list}
...
function project_get_bug_count($p_project_id)
{
	$t_query = new DbQuery();
	$t_query->sql(
		'SELECT COUNT(*) FROM {bug} WHERE project_id='
